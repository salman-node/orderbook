...
//     pair_id: 5,
//     price:  Math.floor(Math.random() * (7508450.2 - 7508350.2 + 1)) + 7508350.2, // Random price between 90000 and 92000
//     quantity: Math.floor(Math.random() * (0.0003 - 0.0001 +0.0001)) + 0.0001, // Random quantity between 1 and 10
//     // price:800,
//     // quantity:5,
//     order_type: 'limit',
//   };  
...
const placeOrder = async () => {
  // Generate random price & quantity
  const price = Math.random() * (7508450.2 - 7508350.2) + 7508350.2;
  const quantity = parseFloat((Math.random() * (0.0003 - 0.0001) + 0.0001).toFixed(4));
  ...
  const buyOrder = {
    ...
    quantity:parseFloat((Math.random() * (0.0003 - 0.0001) + 0.0001).toFixed(4)),
    order_type: 'limit',
  };
  ...
  try {
    ...
    await Promise.all([
      axios.post(url, buyOrder),
      axios.post(url, sellOrder),
    ]);

    console.log('Buy & Sell orders placed:', { price, quantity });
  } catch (error) {
